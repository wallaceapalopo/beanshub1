...
const BeanDetailsModal: React.FC<BeanDetailsModalProps> = ({
  bean,
  onClose,
}) => {
  ...
  const beanUsage = roastingSessions.filter(
    (session) => session.greenBeanId === bean.id
  );
  const totalUsed = beanUsage.reduce(
    (sum, session) => sum + session.greenBeanQuantity,
    0
  );
  const totalProduced = beanUsage.reduce(
    (sum, session) => sum + session.roastedQuantity,
    0
  );
  ...
  const directSales = sales.filter(
    (sale) => sale.productId === bean.id && sale.productType === 'green'
  );
  const totalSalesRevenue = directSales.reduce(
    (sum, sale) => sum + sale.totalAmount,
    0
  );
  const totalSalesQuantity = directSales.reduce(
  ...
  const usageRate =
    originalQuantity > 0
      ? ((totalUsed + totalSalesQuantity) / originalQuantity) * 100
      : 0;

  // Days since entry
  const daysSinceEntry = Math.floor(
    (new Date().getTime() - bean.entryDate.getTime()) / (1000 * 3600 * 24)
  );
              ...
              <div className="bg-gray-50 p-4 rounded-lg space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {bean.entryDate.toLocaleDateString('id-ID')}
                  </span>
                </div>
                <div className="flex justify-between">
            ...
            {beanUsage.length > 0 ? (
              <div className="bg-gray-50 rounded-lg overflow-hidden">
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {beanUsage.slice(-5).map((session) => {
                        const yieldPercentage =
                          (session.roastedQuantity /
                            session.greenBeanQuantity) *
                          100;
                        return (
                ...
                {beanUsage.length > 5 && (
                  <div className="px-4 py-3 bg-gray-100 text-sm text-gray-600 text-center">
                    Menampilkan 5 sesi terakhir dari {beanUsage.length} total
                    sesi
                  </div>
                )}
              </div>
              ...
              <div className="bg-gray-50 p-8 rounded-lg text-center">
                ...
                <p className="text-gray-600">
                  Belum ada riwayat penggunaan untuk biji kopi ini
                </p>
              </div>
            )}
          </div>
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-xl font-bold text-green-800">
                      ...
                      {totalSalesQuantity > 0
                        ? (
                            totalSalesRevenue / totalSalesQuantity
              ...
              <div className="space-y-2">
                <div className="flex justify-between">
                  <span className="text-amber-700">
                    Kuantitas Awal (estimasi):
                  </span>
                  <span className="font-medium">
                    {originalQuantity.toFixed(1)} kg
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-amber-700">Total Digunakan:</span>
                  <span className="font-medium">
                    {(totalUsed + totalSalesQuantity).toFixed(1)} kg
                  </span>
                </div>
                <div className="flex justify-between">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    Rp{' '}
                    {(
                      originalQuantity * bean.purchasePricePerKg
                    ).toLocaleString('id-ID')}
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {totalSalesQuantity > 0 && bean.purchasePricePerKg > 0
                      ? `${(
                          ((totalSalesRevenue -
                            totalSalesQuantity * bean.purchasePricePerKg) /
                            (totalSalesQuantity * bean.purchasePricePerKg)) *
                          100
                        ).toFixed(1)}%`
                      : 'N/A'}
